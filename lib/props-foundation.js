...
/**
 * Three-state props capability for /api/sports discovery.
 *
 * propsStatus:
 *   available   — cache warm and inventory count > 0 (hasProps=true)
 *   unavailable — not props-capable, OR cache warm with zero inventory (hasProps=false)
 *   unknown     — props-capable but cache never warmed / cold start (hasProps=null)
 *
 * Critical: cold cache must NOT report hasProps=false (that deadlocks FE discovery).
 * Cache is an optimization hint — never the sole source of truth for hiding discovery.
 *
 * opts.cacheKnown — true only when _PROPS_RESPONSE_CACHE has an entry for this sport
 */
function sportPropsCapability(sportKey, selectionCount, opts) {
  opts = opts || {};
  var key = _lc(sportKey);
  var capable = PROPS_CAPABLE_SPORTS.indexOf(key) >= 0;
  var count = Number(selectionCount) || 0;
  var cacheKnown = opts.cacheKnown === true;
  if (!capable) {
    return {
      hasProps: false,
      propsStatus: 'unavailable',
      propsCount: 0,
      propsCapable: false
    };
  }
  // Cold / never fetched: discoverable unknown — NOT false.
  if (!cacheKnown) {
    return {
      hasProps: null,
      propsStatus: 'unknown',
  ...
  if (count > 0) {
    return {
      hasProps: true,
      propsStatus: 'available',
      propsCount: count,
      propsCapable: true
    };
  }
  // Warm cache, zero inventory — confirmed unavailable for this moment.
  return {
    hasProps: false,
    propsStatus: 'unavailable',
    propsCount: 0,
...
function _normTeamToken(name) {
  return String(name || '').trim().toLowerCase().replace(/\s+/g, ' ');
}

function _teamTokensMatch(query, candidate) {
  var q = _normTeamToken(query);
  if (!q) return true;
  var c = _normTeamToken(candidate);
  if (!c) return false;
  return c === q || c.indexOf(q) >= 0 || q.indexOf(c) >= 0;
}

function _propGameIdentityKey(p) {
  if (!p) return '';
  var gid = p.gameId || p.providerGameId || p.eventId || p.canonicalGameKey || '';
  if (gid) return 'id:' + String(gid);
  var teams = propHomeAwayTeams(p);
  if (teams.away || teams.home) return 'teams:' + _normTeamToken(teams.away) + '@' + _normTeamToken(teams.home);
  return '';
}

function propHomeAwayTeams(p) {
  var home = p && p.home;
  var away = p && p.away;
  if (home && away) return { home: home, away: away };
  var parsed = parseTeamsFromPropGameId(
    (p && (p.gameId || p.providerGameId || p.eventId || p.canonicalGameKey)) || ''
  );
 ...
 */
function parseTeamsFromPropGameId(gameId) {
...
function filterPropsByExactGameId(props, gameId) {
  ...
  return (props || []).filter(function(p) {
    ...
    if (String(p.eventId || '') === gid) return true;
...
/**
 * Home/away match with unique-game fail-closed semantics.
 * Returns { status: 'unique'|'ambiguous'|'none', props }
 */
function filterPropsByTeamsUnique(props, home, away) {
  var h = home != null && String(home).trim() !== '' ? String(home) : '';
  var a = away != null && String(away).trim() !== '' ? String(away) : '';
  if (!h && !a) return { status: 'none', props: props || [] };
  var matched = (props || []).filter(function(p) {
    if (!p) return false;
    var teams = propHomeAwayTeams(p);
    return _teamTokensMatch(h, teams.home) && _teamTokensMatch(a, teams.away);
  });
  if (!matched.length) return { status: 'none', props: [] };
  var keys = Object.create(null);
  for (var i = 0; i < matched.length; i++) {
    var k = _propGameIdentityKey(matched[i]);
    if (k) keys[k] = true;
  }
  var uniqueCount = Object.keys(keys).length;
  if (uniqueCount > 1) return { status: 'ambiguous', props: [] };
  return { status: 'unique', props: matched };
}

/**
 * Deterministic game-scope resolution for /api/props.
 *
 * Order:
 *   1) Exact provider-native IDs (gameId + optional hints.eventId/providerGameId/canonicalGameKey)
 *   2) Home/away unique fallback only
 *   3) Ambiguous home/away → fail closed (empty)
 *
 * opts.hints may carry odds-cache enrichment when the FE sent a numeric board id.
 */
function resolvePropsForGameScope(props, opts) {
  opts = opts || {};
  var list = props || [];
  var gameId = opts.gameId != null && String(opts.gameId).trim() !== ''
    ? String(opts.gameId) : null;
  var home = opts.home != null ? String(opts.home) : '';
  var away = opts.away != null ? String(opts.away) : '';
  ...
  pushId(gameId);
  pushId(hints.eventId);
  pushId(hints.providerGameId);
  pushId(hints.canonicalGameKey);
  pushId(hints.gameId);

  for (var i = 0; i < ids.length; i++) {
    var hit = filterPropsByExactGameId(list, ids[i]);
    if (hit.length) {
      return { props: hit, filterMode: 'gameId', deniedReason: null };
    }
  }

  var h = home || hints.home || '';
  var a = away || hints.away || '';
  if (h || a) {
    var byTeams = filterPropsByTeamsUnique(list, h, a);
    if (byTeams.status === 'unique') {
      return { props: byTeams.props, filterMode: 'teams', deniedReason: null };
    }
    if (byTeams.status === 'ambiguous') {
      return {
        props: [],
        filterMode: 'ambiguous_denied',
        deniedReason: 'ambiguous_home_away'
      };
    }
  }

  if (gameId || ids.length) {
    return { props: [], filterMode: 'gameId', deniedReason: null };
  }
  return { props: list, filterMode: null, deniedReason: null };
}
...
module.exports = {
  ...
  parseTeamsFromPropGameId: parseTeamsFromPropGameId,
  propHomeAwayTeams: propHomeAwayTeams,
  filterPropsByExactGameId: filterPropsByExactGameId,
  filterPropsByTeamsUnique: filterPropsByTeamsUnique,
  resolvePropsForGameScope: resolvePropsForGameScope,
