...
const VERIFIED_ALIASES = {
  ...
  // FCS / board short-name recoveries (verified unique ESPN targets)
...
function buildAliasesForTeam(team, allTeams) {
  ...
  // Odds feeds often use "FIU Panthers" / "Abilene Chrstn" style labels.
  if (team.abbreviation && team.name) add(team.abbreviation + ' ' + team.name);
  if (team.shortDisplayName && team.name) add(team.shortDisplayName + ' ' + team.name);
  if (team.nickname && team.name && team.nickname !== team.name) {
    add(team.nickname + ' ' + team.name);
  }
  ...
  // shortDisplayName / nickname uniqueness (critical for FCS odds short names)
  var sdKey = normKey(team.shortDisplayName);
  if (sdKey) {
    var sdHits = allTeams.filter(function (t) { return normKey(t.shortDisplayName) === sdKey; });
    if (sdHits.length === 1) add(team.shortDisplayName);
  }
  var nickKey = normKey(team.nickname);
  if (nickKey) {
    var nickHits = allTeams.filter(function (t) { return normKey(t.nickname) === nickKey; });
    if (nickHits.length === 1) add(team.nickname);
  }
...
/**
 * Require a reachable ESPN (or CDN) image before seeding/updating logo_url.
 * Wrong or empty assets are worse than missing — fail closed on non-images.
 */
function validateLogoUrl(url, timeoutMs) {
  return new Promise(function (resolve) {
    var href = String(url || '').trim();
    if (!href || href.indexOf('https://') !== 0) return resolve({ ok: false, reason: 'invalid_url' });
    if (href.indexOf('espncdn.com') < 0 && href.indexOf('espn.com') < 0) {
      return resolve({ ok: false, reason: 'non_espn_host' });
    }
    execFile(
      'curl',
      [
      ...
      { timeout: (timeoutMs || 8000) + 2000, maxBuffer: 64 * 1024 },
      function (err, stdout) {
        if (err || !stdout) return resolve({ ok: false, reason: 'fetch_failed' });
        var parts = String(stdout).trim().split(/\s+/);
        var code = parseInt(parts[0], 10) || 0;
        var ctype = String(parts[1] || '').toLowerCase();
        var size = parseInt(parts[2], 10) || 0;
        if (code !== 200) return resolve({ ok: false, reason: 'http_' + code, httpStatus: code });
        if (ctype.indexOf('image/') !== 0 && ctype.indexOf('octet-stream') < 0) {
          return resolve({ ok: false, reason: 'not_image', contentType: ctype });
        }
        if (size > 0 && size < 64) return resolve({ ok: false, reason: 'too_small', bytes: size });
        resolve({ ok: true, httpStatus: code, contentType: ctype, bytes: size });
      }
    );
  });
}
...
async function syncNcaafTeamLogos(sb, opts) {
  ...
  var skipImageValidation = !!opts.skipImageValidation;
  ...
  if (fetched.fcsLeaks && fetched.fcsLeaks.length) {
    ...
    fcsLeakCount = fetched.fcsLeaks.length;
  }
  if (includeFcs) {
    try {
      var fcs = await fetchEspnFcsTeams(fetched.season);
      fcsGroupCount = fcs.length;
  ...
  // One school set for FBS+FCS — uniqueness / aliases share the college-football resolver.
  teams = dedupeTeamsById(teams);
  rebuildAliases(teams);

  var inserted = 0, updated = 0, failed = 0, skipped = 0, invalidLogo = 0;
  for (var i = 0; i < teams.length; i++) {
    try {
      var row = rowFromEspnTeam(teams[i]);
      ...
      var r = await upsertTeamLogo(sb, row);
    ...
    if ((i + 1) % 20 === 0) await sleep(40);
  }

  var fcsStored = teams.filter(function (t) { return t.classification === 'fcs'; }).length;
  var fbsStored = teams.filter(function (t) { return t.classification !== 'fcs'; }).length;
  ...
  return {
    ...
    fbs_stored: fbsStored,
    fcs_stored: fcsStored,
    fcs_leaks: fcsLeakCount,
    fcs_group: fcsGroupCount,
    inserted: inserted,
    updated: updated,
    failed: failed,
    skipped: skipped,
    invalid_logo: invalidLogo
  };
}
