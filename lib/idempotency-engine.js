  ...
  return function requireIdempotency(opts) {
    ...
    return async function (req, res, next) {
      ...
      res.json = function (body) {
        // Only attach a ticket_id on successful money responses. Error paths
        // (correlation reject, validation, snapshot fail, etc.) must NOT
        // complete/fail with the pre-reserved deterministic ticket id — that
        // would point at a nonexistent ticket and poison exact-retry semantics.
        const statusCode = typeof res.statusCode === 'number' ? res.statusCode : 200;
        const fromBody = body && (body.ticketId || body.groupId) || null;
        const success = statusCode >= 200 && statusCode < 300 && body && body.ok !== false;
        const tid = success
          ? (fromBody || req._idemTicketId || null)
          : (fromBody || null);
        idemComplete(store, scope, statusCode, body, tid).catch(function () {});
