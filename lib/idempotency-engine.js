 ...
 *     vs dual (both column sets during migration).
 *   - Money paths: persistent store required when table exists; fail closed if
 *     table missing. Mem store is NOT primary for money.
...
function createSupabaseStore(getSupabase, opts) {
  ...
  return {
    ...
    async load(scope) {
      const sb = getSupabase && getSupabase();
      if (!sb) return memShadow.load(scope);
      const mode = await probeSchema(sb);
      if (mode === 'missing') return memShadow.load(scope);
    ...
    async insertIfAbsent(scope, row) {
      const sb = getSupabase && getSupabase();
      if (!sb) {
        return memShadow.insertIfAbsent(scope, row);
      }
      const mode = await probeSchema(sb);
      if (mode === 'missing') {
...
async function idemCheck(store, scope, endpoint, body, opts) {
  ...
  if (money && store.ensureSchema) {
    ...
    if (mode === 'missing') {
      // Dual-mode deploy order: table may not exist yet → fail closed for money
      return {
        action: 'reject',
        status: 503,
  ...
  if (existing && existing.expires_at && nowMs > new Date(existing.expires_at).getTime()) {
    if (opts.lookupLedger) {
      const ledger = await opts.lookupLedger(clubId, playerId, clientKey);
      ledgerExists = !!ledger;
      if (ledgerExists) {
        const synth = synthesizeReplayFromLedger(ledger, existing);
    ...
    // No row but ledger may exist (crash after money before complete)
    const ledger = await opts.lookupLedger(clubId, playerId, clientKey);
    if (ledger) {
      const synth = synthesizeReplayFromLedger(ledger, null);
  ...
  if (second.action === 'reserve') {
    // Row exists but evaluate said reserve (shouldn't) — treat as in_progress
 ...
 * opts.money — fail-closed club/player + persistent store; no mem-primary
 */
