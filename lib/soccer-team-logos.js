...
const ESPN_LEAGUE_SLUGS = [
  ...
  'arg.2', 'bra.2', 'por.2', 'bel.2', 'tur.2', 'sco.2',   'eng.3', 'eng.4', 'ger.3', 'egy.1', 'swe.2', 'cze.1', 'pol.1', 'rou.2', 'bul.1',
...
const VERIFIED_ALIASES = {
  ...
  // ESPN DB display names use anglicized "FC Cologne" / "Hamburg SV"
  'Cologne': 'FC Cologne',
  'Koln': 'FC Cologne',
  'FC Koln': 'FC Cologne',
  ...
  // Current Owls board aliases → ESPN team_logos displayName (verified unique)
  'Baltika Kaliningrad': 'FC Baltika Kaliningrad',
  ...
  'Nacional da Madeira': 'C.D. Nacional',
...
const AMBIGUOUS_BARE = {
  ...
  athletic: ['Athletic Club'],
  // Bare "Barcelona" can be La Liga FCB or other clubs — fail closed without verified alias
  barcelona: ['Barcelona', 'Barcelona SC'],
  lincoln: ['Lincoln City', 'Lincoln Red Imps', 'Lincoln United'],
  national: [], // never map bare "national"
  draw: [] // moneyline Draw is not a club
};

/**
 * Verified ESPN seeds for current Owls board gaps not yet present in synced leagues.
 * Merged into resolver index in-memory; sync scripts upsert these into team_logos.
 * Never include ambiguous multi-id search hits (e.g. dual Chesterfield rows).
 */
const BOARD_VERIFIED_SEEDS = [
  { id: '10207', displayName: 'Al Ahly', aliases: ['Al Ahly'] },
  { id: '128', displayName: 'Hansa Rostock', aliases: ['Hansa Rostock'] },
  { id: '6758', displayName: 'Mlada Boleslav', aliases: ['Mlada Boleslav'] },
  { id: '3308', displayName: 'MSV Duisburg', aliases: ['Duisburg', 'MSV Duisburg'] },
  { id: '6367', displayName: 'FC Ingolstadt 04', aliases: ['Ingolstadt', 'FC Ingolstadt 04'] },
  { id: '11017', displayName: 'SSV Jahn Regensburg', aliases: ['Jahn Regensburg', 'SSV Jahn Regensburg'] },
  // Phase-2: current Owls board gaps with verified ESPN id + working crest URL
  { id: '6017', displayName: 'Aucas', aliases: ['Aucas'] },
  { id: '1011', displayName: 'Delfín', aliases: ['Delfin', 'Delfín'] },
  { id: '4812', displayName: 'Deportivo Cuenca', aliases: ['Deportivo Cuenca'] },
  { id: '2668', displayName: 'Emelec', aliases: ['Emelec'] },
  { id: '11124', displayName: 'Guayaquil City FC', aliases: ['Guayaquil City', 'Guayaquil City FC'] },
  { id: '17360', displayName: 'Indy Eleven', aliases: ['Indy Eleven'] },
  { id: '4816', displayName: 'Liga de Quito', aliases: ['LDU Quito', 'Liga de Quito'] },
  { id: '18439', displayName: 'Macará', aliases: ['Macara', 'Macará'] },
  { id: '10307', displayName: 'Manta F.C.', aliases: ['Manta', 'Manta F.C.'] },
  { id: '17176', displayName: 'Mushuc Runa', aliases: ['Mushuc Runa'] },
  { id: '20695', displayName: 'Orense', aliases: ['Orense'] },
  { id: '9867', displayName: 'Preußen Münster', aliases: ['Preussen Munster', 'Preußen Münster'] },
  { id: '18448', displayName: 'Real Monarchs SLC', aliases: ['Real Monarchs', 'Real Monarchs SLC'] },
  { id: '18741', displayName: 'Saarbrücken', aliases: ['Saarbrucken', 'Saarbrücken'] },
  { id: '18327', displayName: 'Slovacko', aliases: ['Slovacko'] },
  { id: '9292', displayName: 'Técnico Universitario', aliases: ['Tecnico Universitario', 'Técnico Universitario'] },
  { id: '2687', displayName: 'The Strongest', aliases: ['The Strongest'] },
  { id: '17491', displayName: 'Würzburger Kickers', aliases: ['Wurzburger Kickers', 'Würzburger Kickers'] }
];
...
async function syncSoccerTeamLogos(sb, opts) {
  ...
  // Ensure board-verified seeds are always upserted (exact ESPN IDs only).
  BOARD_VERIFIED_SEEDS.forEach(function (seed) {
    if (!seed || !seed.id) return;
    if (!teams.some(function (t) { return String(t.id) === String(seed.id); })) {
      teams.push({
        id: String(seed.id),
        displayName: seed.displayName,
        name: seed.displayName,
        abbreviation: '',
        logo: ESPN_SOCCER_LOGO(seed.id),
...
function buildResolverIndex(rows) {
  ...
  merged.forEach(function (r) {
    if (r && r.provider_team_id != null) knownIds[String(r.provider_team_id)] = true;
  });
  BOARD_VERIFIED_SEEDS.forEach(function (seed) {
    if (!seed || !seed.id || knownIds[String(seed.id)]) return;
    merged.push({
      sport: SPORT,
      provider: PROVIDER,
      provider_team_id: String(seed.id),
      canonical_name: seed.displayName,
      display_name: seed.displayName,
      logo_url: ESPN_SOCCER_LOGO(seed.id),
      aliases: seed.aliases || [],
      classification: 'club',
      active: true,
      _seed: true
