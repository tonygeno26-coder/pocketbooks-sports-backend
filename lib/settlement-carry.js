'use strict';
/**
 * Carried settlement balance (cash position) — pure helpers.
 *
 * Sign convention (player POV):
 *   negative = player owes host
 *   positive = host owes player
 *   zero     = settled
 *
 * Authoritative derivation (no parallel client math):
 *   carry = ticketSettledNet(after epoch) + Σ player_paid_host - Σ host_paid_player
 *
 * Host settlement moves toward zero only and must not cross zero.
...
/**
 * Apply a host cash settlement toward zero.
...
function applyPartialSettlement(currentBalance, settlementAmount) {
  ...
  if (amt > maxAmount + 0.01) {
    return {
      ...
      maxAmount: maxAmount,
      attempted: amt,
      message: 'Settlement cannot cross zero. Max applicable is $' + maxAmount.toFixed(2)
    };
  }
...
/**
 * Derive carried settlement balance from ticket nets + confirmed payments.
 * @param {number} ticketSettledNet  player POV (+ host owes / − player owes)
 * @param {number} playerPaidHost    confirmed cash player→host after epoch
 * @param {number} hostPaidPlayer    confirmed cash host→player after epoch
 */
function deriveSettlementCarry(ticketSettledNet, playerPaidHost, hostPaidPlayer) {
  return rnd(
 ...
 * New rollovers must NOT write these; new cash settlements must NOT advance epoch.
 */
function isHistoricalEpochMarker(row) {
  ...
  if (t === 'SETTLEMENT_APPLIED' || t === 'settlement_applied') {
    ...
    // Rollover-written markers only — cash settle must NOT advance ticket epoch
