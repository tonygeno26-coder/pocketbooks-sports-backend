...
const paySql = fs.readFileSync(path.join(__dirname, '..', 'migrations', 'PROPOSED_settlement_records.sql'), 'utf8');
...
const settleFn = src.slice(
  src.indexOf('async function _handleRecordSettlement'),
  src.indexOf("app.get('/api/host/settlement-records'")
);

console.log('\n── Sign convention matrix ──');

test('−500 + 200 recorded → −300 (player_paid_host)', function() {
...
test('+500 + 200 recorded → +300 (host_paid_player)', function() {
  var r = sc.applyPartialSettlement(500, 200);
  assert.strictEqual(r.after, 300);
...
console.log('\n── Constraints (proposed settlement_records SQL) ──');
...
test('direction check + amount > 0 + cents match', function() {
  ...
  assert.ok(paySql.indexOf('settlement_records_amount_cents_matches') !== -1);
  assert.ok(paySql.indexOf('settlement_records_club_ledger_settlement_uidx') !== -1);
});
...
test('stale −500 preview vs actual −200: record 500 → over_settlement_blocked max 200', function() {
...
test('stale −500 preview vs actual −200: record 200 → ok → 0', function() {
...
test('settle-player recomputes via serialized RPC (never trusts FE balance_before)', function() {
  assert.ok(settleFn.indexOf("_callMoneyRpc('record_settlement_option_a_tx'") !== -1);
...
test('settlement recording gated off unless SETTLEMENT_RECORDING_ENABLED=true', function() {
  assert.ok(src.indexOf("process.env.SETTLEMENT_RECORDING_ENABLED === 'true'") !== -1);
  assert.ok(settleFn.indexOf('settlement_recording_disabled') !== -1);
});
...
test('advisory lock + lock_timeout wiring present', function() {
  ...
  assert.ok(fs.existsSync(path.join(__dirname, '..', 'migrations', 'PROPOSED_record_settlement_option_a_tx.sql')));
});

test('concurrent over-settlement race documented: two×400 on −500 must not both succeed unchecked', function() {
  ...
  // If both wrote settlement records: ticketNet -500 + 800 = +300 (WRONG — crossed zero via settlement records)
  ...
  assert.strictEqual(sequential2.error, 'over_settlement_blocked');
  // Serialization RPC must exist to prevent the raced outcome
  var sql = fs.readFileSync(path.join(__dirname, '..', 'migrations', 'PROPOSED_record_settlement_option_a_tx.sql'), 'utf8');
  assert.ok(sql.indexOf('pg_advisory_xact_lock') !== -1);
});

console.log('\n── Idempotency ──');

test('club-scoped record_id and settlementId', function() {
...
test('settle path declares bankrollMutated false and no settle_player_tx', function() {
  ...
  assert.ok(settleFn.indexOf("_callMoneyRpc('record_settlement_option_a_tx'") !== -1);
  assert.ok(paySql.indexOf('settlement_records') !== -1);
});

test('settlement_records comment: does not rewrite ticket bankroll', function() {
  assert.ok(paySql.indexOf('Does not rewrite ticket bankroll') !== -1 ||
            paySql.indexOf('does not rewrite') !== -1);
});

test('epoch cutoff uses historical markers only; settlement record type settlement_record in SQL', function() {
  assert.ok(src.indexOf('isHistoricalEpochMarker') !== -1);
  var settleSql = fs.readFileSync(path.join(__dirname, '..', 'migrations', 'PROPOSED_record_settlement_option_a_tx.sql'), 'utf8');
  assert.ok(settleSql.indexOf("'settlement_record'") !== -1);
